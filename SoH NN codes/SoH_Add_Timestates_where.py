# -*- coding: utf-8 -*-
"""
Created on Wed Nov 24 10:08:57 2021
...
start=time.time()
#x="D:\\Benisha\\SoH_NN\\Output\\testfile_1.csv"#, sep='\t'
#data1 = pd.read_csv(x,header=0,index_col=False,error_bad_lines=False)   # Replace x with this format, in case you need to run this code alone; "D:\\Benisha\\Battery DCA\\Mohali_data1\\Original_data1_files\\2019_M4_Apr_driving_temperature_voltage_sorted.csv"

#%% Calculating the times for which the cells are kept in different voltage states
def timestates(data1,file_path,summary_path,key):
    ...
    #    data1[j]=data1[j].where(data1[j]!=119,0)
        data1[j]=data1[j].where(data1[j]<60,0)
    ...
    data1=data1[data1.T_Max<100]  #& (data1.T_Min>0)
    data=pd.DataFrame()
...
#        try:
#            result[i]['Capacity']=result[i]['current']*result[i]['Time_in_mins_s']/60
#            result[i]['C_rate_inst']=((result[i]['current']/result[i]['Capacity'])).cumsum()
#    #        result[i]['C_rate']=result[i].mode()['C_rate_inst']
#        except KeyError:
#            result[i]['Capacity']=result[i]['instCurrent']*result[i]['Time_in_mins_s']/60
#            result[i]['C_rate_inst']=((result[i]['instCurrent']/result[i]['Capacity'])).cumsum()
#    #        result[i]['C_rate']=(result[i]['C_rate_inst']).mean()
#            
    #%%    
        result[i]['Vol_s0_mins']=np.where(result[i]['Vol_State_No']==0,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s1_mins']=np.where(result[i]['Vol_State_No']==1,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s2_mins']=np.where(result[i]['Vol_State_No']==2,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s3_mins']=np.where(result[i]['Vol_State_No']==3,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s4_mins']=np.where(result[i]['Vol_State_No']==4,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s5_mins']=np.where(result[i]['Vol_State_No']==5,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s6_mins']=np.where(result[i]['Vol_State_No']==6,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s7_mins']=np.where(result[i]['Vol_State_No']==7,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Vol_s8_mins']=np.where(result[i]['Vol_State_No']==8,result[i]['Time_in_mins_s'],np.nan)
    
        result[i]['Temp_s0_mins']=np.where(result[i]['Temp_State_No']==0,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s1_mins']=np.where(result[i]['Temp_State_No']==1,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s2_mins']=np.where(result[i]['Temp_State_No']==2,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s3_mins']=np.where(result[i]['Temp_State_No']==3,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s4_mins']=np.where(result[i]['Temp_State_No']==4,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s5_mins']=np.where(result[i]['Temp_State_No']==5,result[i]['Time_in_mins_s'],np.nan)
        result[i]['Temp_s6_mins']=np.where(result[i]['Temp_State_No']==6,result[i]['Time_in_mins_s'],np.nan)
    ...
    df1['Time_Cumulative_Estimate']=df1[['Vol_s0_mins','Vol_s1_mins','Vol_s2_mins','Vol_s3_mins','Vol_s4_mins','Vol_s5_mins','Vol_s6_mins','Vol_s7_mins','Vol_s8_mins']].sum(axis=1)
    ...
    df.to_csv(file_path+'\\'+key+'_Combined_sessions_timestates.csv')         
