# -*- coding: utf-8 -*-
"""
Created on Fri Dec  6 13:06:38 2019
...
files = glob.glob("C:\\Users\\IITM\\Desktop\\AMBEDKAR\\PHY13\\cccv\\datalogger\\*.csv") # path to the Folder in which the data logger files are stored
...
df.to_csv("D:\\Benisha\\dataloggermerged1.csv") #use any path and file name here
df=df.reset_index()
print('...........1/3')
...
with open("C:\\Users\\IITM\\Desktop\\AMBEDKAR\\PHY13\\cccv\\first11.pkl",'rb') as f: # path to the .pkl file which has been processed from the .nda file
    data=pkl.load(f)
...
#%%  
    y1=data[data.Timeflag==False].index.tolist()
    #%%
    for a in range(0,17): # iterating channel names
        for c,i in enumerate (y1):
...
#%%    
    ...
    data=data.drop(['Realtime','Flag','Timeflag'],axis=1)
    data=data.dropna(subset=['RTC'])
    print('...........3/3')
    data.to_pickle("D:\\Benisha\\finaldoc_TimebaseMatched5.pkl")
    data.to_csv("D:\\Benisha\\finaldoc_TimebaseMatchedtest5.csv")
