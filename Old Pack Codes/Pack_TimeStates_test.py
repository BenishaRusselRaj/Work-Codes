# -*- coding: utf-8 -*-
"""
Created on Wed Nov 27 16:57:57 2019

@author: IITM
"""
"""
to add time spent in each vtg state for a battery pack
...
#    data1_s=[pd.read_csv(f, header=0, index_col=False,sep='\t',error_bad_lines=False) for f in files]
#    data1=pd.concat(data1_s, ignore_index=True)
#    
#    temp=path.split('\\')
#    fin_path=''
#    for i in range(0, len(temp)-1):
#        fin_path=fin_path+temp[i]+'\\'
#    
#    ##
#    #%%
#      # Change this number according to the position of the folder name having bin number; Do this only if bin number is not present in original data
#    ##
#    
...
#    
    ...
    data1['V_Min']=data1.loc[:,'CV0':'CV12'].min(axis=1)
    ...
    data1['Maximum_Voltage_cell']=data1.loc[:,'CV0':'CV12'].idxmax(axis=1)
    df1=pd.DataFrame(columns=data1.columns,index=data1.index)
    df=pd.DataFrame()
    
    data1['Avg_Temp']=data1.loc[:,'T0':'T16'].mean(axis=1)
    data1['Avg_Vtg']=data1.loc[:,'CV0':'CV12'].mean(axis=1)
    print('1/4............')
    ...
    x=x.split('.')
    df.to_csv(x[0]+'Combined_timestates.csv')         
    df1.to_csv(x[0]+'Timestates_sessionSummary.csv')  
    return (x[0]+'Timestates_sessionSummary.csv')
    #
#    df.to_csv(fin_path+'Combined_timestates.csv')         
#    df1.to_csv(fin_path+'Timestates_sessionSummary.csv')  
#    return (fin_path+'Timestates_sessionSummary.csv')
