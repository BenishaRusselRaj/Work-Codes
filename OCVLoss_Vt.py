# -*- coding: utf-8 -*-
"""
Created on Thu Oct 17 19:23:47 2019
    ...
    for name,group in grouped:
        ...
        if ((group.Step_Type=='CCCV_Chg').all()):
            for a in range(0,6):
                ...
                group['SOC']=(group['SOC']).astype(float)
                group['V_'+x[a]+'C']=(df_Chg['V_'+x[a]+'C']).astype(float).truncate(before=None,after=j)
                group['SOC_'+x[a]+'C']=df_Chg['SOC_'+x[a]+'C'].astype(float).truncate(before=None,after=j)
                group['Vt_Chg'+x[a]+'C'] = np.where((group['SOC'])==(group['SOC_'+x[a]+'C']), group['V_'+x[a]+'C'], np.nan)
            ...
            for a in range(0,6):
                ...
                group=group.fillna(0)
                group['SOC_'+x1[a]+'C']=(df_DChg['SOC_'+x1[a]+'C']).astype(float)
                group['SOC']=(group['SOC']).astype(float)
                group['V_'+x1[a]+'C']=(df_DChg['V_'+x1[a]+'C']).astype(float).truncate(before=None,after=j)
                group['SOC_'+x1[a]+'C']= df_DChg['SOC_'+x1[a]+'C'].astype(float).truncate(before=None,after=j)
                group['Vt_DChg'+x1[a]+'C'] = np.where((group['SOC'])==(group['SOC_'+x1[a]+'C']), group['V_'+x1[a]+'C'], np.nan)
            group=group.fillna(method='ffill')
            for a in range(0,6):
