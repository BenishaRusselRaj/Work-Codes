# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 18:23:07 2020

@author: IITM
"""


def combinedata_files(path1,path2, folder_path):
    import pandas as pd
    import numpy as np
#    from statistics import mode
#    import statistics
    
    df11=pd.read_csv(path1, sep=',',header=0,low_memory=False) # Charging sessions summary alone  # "D:\\Benisha\\Code files\\Battery_Pack_mohali_Charging_Data_M4_Apr_timestates_sessionSummary_Time_filtered.csv"
    df12=pd.read_csv(path2,sep=',',header=0,low_memory=False) # discharging sessions summary alone   # "D:\\Benisha\\Code files\\Battery_Pack_mohali_Driving_Data_M4_Apr_timestates_sessionSummary_Time_filtered.csv"
...
#        pass
    ...
    for i in range(0, len(df1f)):
        ...
        df1f[i]['Cycle_No_Chg_DChg']=(df1f[i]['Chg_flag'].diff()>0).cumsum()
        df1f[i]['Cycle_No_session']=(df1f[i]['session']!=df1f[i]['session'].shift()).cumsum()
...
#            df1f[i].loc[y1[j],'start_current']=df1f[i].loc[y1[j]-1,'end_current']
#            df1f[i].loc[y1[j],'end_current']=df1f[i].loc[y1[j]+1, 'start_current'] 
#            df1f[i].loc[y1[j],'start_pack_vtg']=df1f[i].loc[y1[j]-1,'end_pack_vtg']
#            df1f[i].loc[y1[j],'end_pack_vtg']=df1f[i].loc[y1[j]+1, 'start_pack_vtg'] 
    ...
    data=data.drop(['Unnamed: 0.1','Chg_flag','Flag_Time_diff'],axis=1)
    
    data.to_csv(folder_path+'\\Combined_cyclesData_filtered_test.csv')
