...
def PHY13_Cal_Degradation_model_fitting():
    temp = np.array([25,35,45,55])
    deg_soc_60 = np.array([0.76,2.6395,8.0173,33.9502])
    deg_soc_80 = np.array([1.3,4.8229,7.8807,32.8101])
    deg_soc_100 = np.array([1.97,5.3160,10.5025,39.2598])
...
def PHY13_Cyc_Degradation_model_fitting():
    temp = np.array([25,35,45])
    deg_dod_80 = np.array([20/1500 , 20/980, 20/920])
    deg_dod_100 = np.array([20/1320, 20/750, 20/500])
...
def PHY13_Cal_Degradation_from_SOC_Temperature(SOCx, Tx, f1, f2, f3, time_min):
    if Tx < 25: Tx = 25;
    if Tx > 55: Tx = 55;
...
def PHY13_Cyc_Degradation_from_SOC_Temperature(DoDx, Tx, f4, f5, no_of_cycles):
    if Tx < 25: Tx = 25;
    if Tx > 45: Tx = 45;
...
def PHY13_SOH_estimation(df1): #time estimate is in minutes
    f1, f2, f3 = PHY13_Cal_Degradation_model_fitting()
    for i in range (0,len(df1)):
        ...
        for j in range (2,len(df1[i].SOH_states)):
            ...
            if df1[i].Session_Type.iloc[j] == 'Rest':
               ...
               df1[i].Cal_Deg_Prcnt_this_session_avg.iloc[j] =  PHY13_Cal_Degradation_from_SOC_Temperature(SOCx, Tx_avg, f1, f2, f3, df1[i].Time_Estimate.iloc[j]);
               df1[i].Cal_Deg_Prcnt_this_session_min.iloc[j] =  PHY13_Cal_Degradation_from_SOC_Temperature(SOCx, Tx_min, f1, f2, f3, df1[i].Time_Estimate.iloc[j]);
               df1[i].Cal_Deg_Prcnt_this_session_max.iloc[j] =  PHY13_Cal_Degradation_from_SOC_Temperature(SOCx, Tx_max, f1, f2, f3, df1[i].Time_Estimate.iloc[j]);
...
def PHY13_Plot_Cal_Degradation_Contour():
    ...
    for i in range(len(Tnew)):
        ...
        for j in range(len(SOCnew)):
            Degarr[i][j], Life_yrs[i][j]  = PHY13_Cal_Degradation_from_SOC_Temperature(SOCnew[j], Tnew[i], f1, f2, f3, 90*24*60);
    ...
    plt.savefig('PHY13_Cal_Deg.png', format='png', dpi=1200)
    ...
    plt.savefig('PHY13_cal_life.png', format='png', dpi=1200)


def PHY13_Plot_Cyc_Degradation_Contour():
    ...
    for i in range(len(Tnew)):
        ...
        for j in range(len(DoDnew)):
            Degarr[i][j], Life_cycles[i][j]  = PHY13_Cyc_Degradation_from_SOC_Temperature(DoDnew[j], Tnew[i], f4, f5, 90*24*60);
    ...
    plt.savefig('PHY13_Cyc_Deg.png', format='png', dpi=1200)
    ...
    plt.savefig('PHY13_cyc_life.png', format='png', dpi=1200)