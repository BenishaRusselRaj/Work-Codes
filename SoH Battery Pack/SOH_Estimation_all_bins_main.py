...
import numpy as np
import sys
sys.path.append("D:\\Sushant\\soh\\Code") # path to code file
...
import Cell_Degradation_Modelling
...
def SOH_estimation(file_path):
    ...
    df['Avg_temp'] = df[["T_pack_Avg"]].max(axis=1)

    #df['Avg_temp'] = df[["start_Avg_Temp","end_Avg_Temp"]].max(axis=1) #key error; what is done in plae of this

    df.loc[df['Session_Type'] == 'Rest', 'Avg_temp'] = df[['T_pack_Avg']].min(axis=1) ; # - 2
    df=df[df.Time_Estimate<=178560] # time in minutes less than 4 months


    cols=['bin','start_Time','end_Time','nc_bms','SOH_BMS','nc_otd','SOH_otd','nc_cell_testing','SOH_cell_testing_avg','SOH_cell_testing_min','SOH_cell_testing_max']
    ...
    try:
        ...
        result = SOH_estimation_by_Temperature_Time_1.PHY13_SOH_estimation_by_Temperature_Time(result.copy()) # model 1
        #result = Cell_Degradation_Modelling.PHY13_SOH_estimation(result.copy())  # model 2
    ...
    #%% To generate file separately
        ...
        for i in range (0,len(result)):
            if(len(result[i])>0):
                ...
                data.iloc[k]['SOH_cell_testing_avg']=100-result[i].iloc[-1]['Total_Degradation_avg']
                data.iloc[k]['SOH_cell_testing_min']=100-result[i].iloc[-1]['Total_Degradation_min']
                data.iloc[k]['SOH_cell_testing_max']=100-result[i].iloc[-1]['Total_Degradation_max']
    ...
    #                data['SOH_cell_testing']=100-result[i]['Total_Degradation_avg']
    #
    ...
    #%%
        ...
        #            bin_no = result[i][0:1]
                ...
                # plt.scatter(gp['end_Time'],100-gp.Total_Degradation_avg)
                ...
                plt.scatter(gp['Cycle_No_session'],100-gp.Total_Degradation_avg)
        ...
        try:
            for i in range (0,len(result)):
                ...
                # for BMS prredn
                    ...
                    Y3 = 100-gp.Total_Degradation_avg
